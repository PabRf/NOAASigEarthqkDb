...
test_that("eq_clean_data", {
  data = readr::read_csv("earthquakes_-2150to2024_clean1.csv", col_names = T, progress = FALSE)
  data = eq_clean_data("earthquakes_-2150to2024_clean1.csv")
  expect_equal(data$`Location Name`[[1]], "Nisa")
})

test_that("eq_filter_data", {
  data = readr::read_csv("earthquakes_-2150to2024_clean2.csv", col_names = T, progress = FALSE)
  data = eq_filter_data("earthquakes_-2150to2024_clean2.csv", xmin = as.Date("1950-01-01", "%Y-%m-%d"),
                        xmax = as.Date("1955-01-01", "%Y-%m-%d"), c("Date", "xend", "y", "yend", "Mag", "Deaths", "Country"))
  expect_equal(nrow(data), 36)
})

test_that("geom_timeline", {
  data = readr::read_csv("earthquakes_-2150to2024_final.csv", col_names = T, progress = FALSE)
...
test_that("eq_map", {
  ...
  expect_equal(data$x$settings$tooltip$opacity, 0.7)
})
